...
import { getRegExpMatches, isNil, pathToUrl } from "../../../utils";
import {
  AppCompletionItem,
  AppCompletionList,
  CompletionsProvider,
...
export interface CompletionEntryWithIdentifer
  extends ts.CompletionEntry,
    TextDocumentIdentifier {
  position: Position;
}
  ...
  implements CompletionsProvider<CompletionEntryWithIdentifer>
{
  ...
  private isValidTriggerCharacter(
    ...
    return this.validTriggerCharacters.includes(
      character as validTriggerCharacter
    );
  }
  ...
  private getExistingImports(document: Document) {
    const rawImports = getRegExpMatches(
      scriptImportRegex,
      document.getText()
  ...
  private async getEventAndSlotLetCompletions(
    ...
    const events = componentInfo
      .getEvents()
      .map((event) => mapToCompletionEntry(event, "on:"));
  ...
  private toCompletionItem(
    ...
    const completionLabelAndInsert = this.getCompletionLabelAndInsert(
      fragment,
      comp
    );
    ...
    const textEdit = replacementSpan
      ? TextEdit.replace(
          convertRange(fragment, replacementSpan),
          insertText ?? label
        )
      : undefined;
  ...
  private getCompletionLabelAndInsert(
    ...
    if (isScriptElement && hasModifier) {
      const label =
        kindModifiers && !name.endsWith(kindModifiers)
          ? name + kindModifiers
          : name;
      return {
        insertText: name,
  ...
  private fixTextEditRange(
    ...
    textEdit.newText = newText.substring(
      wordRangeStartCharacter - start.character
    );
...
const beginOfDocumentRange = Range.create(
  Position.create(0, 0),
  Position.create(0, 0)
);
