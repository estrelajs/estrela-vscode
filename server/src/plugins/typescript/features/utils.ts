...
export async function getComponentAtPosition(
  ...
  const generatedPosition = fragment.getGeneratedPosition(
    doc.positionAt(node.start + 1)
  );
 ...
 */
export function isInGeneratedCode(
  text: string,
  start: number,
  end: number = start
) {
...
export function isPartOfImportStatement(
  text: string,
  position: Position
): boolean {
...
export class SnapshotFragmentMap {
  private map = new Map<
    string,
    { fragment: SnapshotFragment; snapshot: DocumentSnapshot }
  >();
  constructor(private resolver: LSAndTSDocResolver) {}

  set(
    fileName: string,
    content: { fragment: SnapshotFragment; snapshot: DocumentSnapshot }
  ) {
...
>(
  (node) =>
    ts.isFunctionDeclaration(node) && node?.name?.getText() === "render",
  ts.isSourceFile
);
...
export const isReactiveStatement =
  nodeAndParentsSatisfyRespectivePredicates<ts.LabeledStatement>(
...
export const isInReactiveStatement = (node: ts.Node) =>
  isSomeAncestor(node, isReactiveStatement);
...
export const gatherIdentifiers = (node: ts.Node) =>
  gatherDescendants(node, ts.isIdentifier);
