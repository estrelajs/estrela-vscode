...
import { filterAsync, isNil, pathToUrl } from "../../../utils";
...
export class RenameProviderImpl implements RenameProvider {
  ...
  async prepareRename(
    ...
    const renameInfo = this.getRenameInfo(
      lang,
      tsDoc,
      document,
      position,
  ...
  private async getAdditionLocationsForRenameOfPropInsideComponentWithProp(
    ...
    const idxOfOldPropName = fragment.text.lastIndexOf(
      ":",
  ...
  private async mapAndFilterRenameLocations(
    ...
    return this.filterWrongRenameLocations(mappedLocations.filter(isNil));
  }
  ...
  private filterWrongRenameLocations(
    ...
    return filterAsync(mappedLocations, async (loc) => {
      ...
      function notPrecededBy(str: string) {
        return (
          content.lastIndexOf(str, loc.textSpan.start) !==
          loc.textSpan.start - str.length
        );
      }
    });
  }
  ...
  private getVariableAtPosition(
    ...
    const offset = fragment.offsetAt(fragment.getGeneratedPosition(position));
    const { start, length } = lang.getSmartSelectionRange(
      tsDoc.filePath,
      offset
    ).textSpan;
