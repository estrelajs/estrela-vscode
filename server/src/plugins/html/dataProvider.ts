...
const estrelaEvents = [
  ...htmlData.globalAttributes!.map(mapToEstrelaEvent),
  // {
  //   name: "on:introstart",
...
const estrelaAttributes: IAttributeData[] = [
  // {
  //   name: "bind:innerHTML",
...
const estrelaTags: ITagData[] = [
  ...
  //   name: "estrela:body",
  //   description:
  //     "As with <estrela:window>, this element allows you to add listeners to events on document.body, such as mouseenter and mouseleave which don't fire on window.",
  //   attributes: [],
  // },
  // {
  //   name: "estrela:head",
  ...
  //           name: "{false}",
  //           description:
  //             "The default. Estrela will be more conservative about whether or not mutable objects have changed",
  //         },
  //       ],
  //     },
  //     {
...
const addAttributes: Record<string, IAttributeData[]> = {
  ...
  a: [
    // {
    //   name: "estrelakit:noscroll",
    //   description:
    //     "EstrelaKit-specific attribute. Will prevent scrolling after the link is clicked.",
    //   valueSet: "v",
    // },
    // {
    //   name: "estrelakit:prefetch",
    //   description:
    //     "EstrelaKit-specific attribute. Will cause EstrelaKit to run the page's load function as soon as the user hovers over the link (on a desktop) or touches it (on mobile), rather than waiting for the click event to trigger navigation.",
    //   valueSet: "v",
    // },
  ],
  details: [
...
const html5Tags = htmlData.tags!.map((tag) => {
  let attributes = tag.attributes.map(mapToEstrelaEvent);
...
export const estrelaHtmlDataProvider = newHTMLDataProvider("estrela-builtin", {
  version: 1,
  globalAttributes: [
  ...
  tags: [...html5Tags, ...estrelaTags],
  valueSets: htmlData.valueSets,
});

function mapToEstrelaEvent(attr: IAttributeData) {
  return {
    ...attr,
