    ...
    SelectionRangeProvider
{
  ...
  constructor(docManager: DocumentManager, configManager: LSConfigManager) {
    ...
    docManager.on("documentClose", (document) =>
      this.cssDocuments.delete(document)
    );
  }
  ...
  getSelectionRange(
    ...
    const [range] = getLanguageService(
      extractLanguage(cssDocument)
    ).getSelectionRanges(
      cssDocument,
  ...
  private inStyleAttributeWithoutInterpolation(
    ...
    return (
      ...
      !text
        .substring(attrContext.valueRange[0], attrContext.valueRange[1])
        .includes("{")
    );
  }
  ...
  getColorPresentations(
    ...
    return getLanguageService(extractLanguage(cssDocument))
      ...
      .map((colorPres) =>
        mapColorPresentationToOriginal(cssDocument, colorPres)
      );
  }
