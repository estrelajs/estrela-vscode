...
import { pathToUrl, isNil } from "../../../utils";
...
export class TypeDefinitionProviderImpl implements TypeDefinitionProvider {
  ...
  async getTypeDefinition(
    ...
    const offset = mainFragment.offsetAt(
      mainFragment.getGeneratedPosition(position)
    );
    ...
    const result = await Promise.all(
      typeDefs.map(async (typeDef) => {
        ...
        if (
          !isNoTextSpanInGeneratedCode(snapshot.getFullText(), typeDef.textSpan)
        ) {
          return;
        }
