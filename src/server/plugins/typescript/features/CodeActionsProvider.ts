...
export class CodeActionsProviderImpl implements CodeActionsProvider {
  ...
  private fixIndentationOfImports(
    ...
    return modifyLines(edit, (line, idx) =>
  ...
  private async applyQuickfix(
    ...
    const errorCodes: number[] = context.diagnostics.map((diag) =>
      Number(diag.code)
    );
        ...
        return TextDocumentEdit.create(
          ...
          change.textChanges
            .map((edit) => {
              ...
              if (
                !isNoTextSpanInGeneratedCode(snapshot.getFullText(), edit.span)
              ) {
                return undefined;
              }
  ...
  private getComponentImportQuickFix(
    ...
    return completion.entries
      .filter((c) => c.name === name || c.name === suffixedName)
      .flatMap(toFix);
  }
      ...
      this.applicableRefactorsToCodeActions(
        ...
        .map((refactor) => ({
          ...
          title: refactor.title
            ...
            .replace(
              "Extract to constant in function 'render'",
              "Extract to constant"
            ),
        }))
    );
  ...
  private checkDisableJsDiagnosticsCodeInsert(
    ...
    const insertedAfterExport = text
      .slice(0, startOffset)
      .trim()
      .endsWith("export");
    ...
    const position = document.positionAt(
      text.lastIndexOf("export", startOffset)
    );
