      ...
      if (is$$PropsError) {
        ...
        if (propsStart) {
          const start = document.positionAt(
            propsStart +
              document.getText().substring(propsStart).indexOf("$$Props")
          );
          range = {
            start,
...
function enhanceIfNecessary(diagnostic: Diagnostic): Diagnostic {
  ...
  if (
    diagnostic.code === DiagnosticCode.JSX_ELEMENT_DOES_NOT_SUPPORT_ATTRIBUTES
  ) {
    return {
      ...diagnostic,
      message:
...
function resolveNoopsInReactiveStatements(
  ...
  const expandedDiagnostics = passMap(
    diagnostics,
    expandRemainingNoopWarnings
  ).flat();
